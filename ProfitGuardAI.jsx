...
function getStockUsage(salesYesterday) {
  let milkUsed = 0, coffeeUsed = 0, pastriesUsed = 0;
  for (const item of MENU_RECIPES) {
...
// Input:  yesterday's sales counts (e.g. {latte:120, flatwhite:45})
//         + standard daily milk order (e.g. 60L)
// Output: order recommendation + savings calculation
function calculateOrderRecommendation(salesYesterday, standardMilkOrder = 60) {
  let milkUsedMl = 0;
  let coffeeUsedG = 0;
  let revenueAed = 0;
  let pastryUnits = 0;
  ...
  for (const item of MENU_RECIPES) {
    const qty = salesYesterday[item.id] || 0;
    if (qty === 0) continue;
    const milkForItem = qty * item.milk_ml;
    const coffeeForItem = qty * item.coffee_g;
    milkUsedMl  += milkForItem;
    coffeeUsedG += coffeeForItem;
    revenueAed  += qty * item.cost_aed;
...
function SalesEntry({ cafeName = "Your Cafe", ownerWhatsApp = "", stock, setStock }) {
  ...
  const handleCalculate = () => {
    ...
    const result = calculateOrderRecommendation(salesNumbers, standardOrder);
    setCalc(result);
    setWhatsappOpened(false);

    // ── AUTO-SUBTRACT FROM STOCK ──
    if (stock && setStock && !stockApplied) {
      const used = getStockUsage(salesNumbers);
      ...
      <div style={{
        ...
        {MENU_RECIPES.map((item) => (
          <div key={item.id} style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: "var(--text-1)", fontWeight: 500 }}>{item.name}</div>
              <div style={{ fontSize: 10, color: "var(--text-3)" }}>
                {item.milk_ml > 0 ? `${item.milk_ml}ml milk · ${item.coffee_g}g coffee` : "no milk"}
              </div>
            </div>
            <input
              type="text"
