...
const getCafe    = () => _cafe;
const setSession = (s) => { _session = s; };
const setCafe    = (c) => { _cafe = c; };
...
/* ── nav ── */
.pg-nav{width:100%;display:flex;background:#06101e;border-bottom:1px solid var(--border);position:sticky;top:0;z-index:80;overflow-x:auto}
.pg-nav::-webkit-scrollbar{height:0}
.pg-nav-tab{flex:1;min-width:72px;padding:13px 6px;font-family:var(--font-d);font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:var(--text-2);background:none;border:none;border-bottom:2px solid transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px;white-space:nowrap;transition:color .18s,border-color .18s}
...
function MunicipalityLog({open,onClose,arabic,onSuccess}){
  ...
  const canSubmit=staff&&staff!==STAFF[0]&&checkedCount>0;
  ...
  const submit=()=>{ setPhase("success"); onSuccess?.(); };
            ...
            <div style={{padding:"16px 18px",display:"flex",flexDirection:"column",gap:"18px"}}>
              ...
              <div>
                ...
                <div style={{position:"relative"}}>
                  <select className="ml-select" value={staff} onChange={e=>setStaff(e.target.value)}>
                    {STAFF.map(s=><option key={s} value={s}>{s}</option>)}
                  </select>
                  <ChevronDown size={14} color="var(--text-2)" style={{position:"absolute",right:"12px",top:"50%",transform:"translateY(-50%)",pointerEvents:"none"}}/>
                </div>
              </div>
    ...
    <>
      ...
      <div className="tbl slide-in">
        <div className="tbl-hrow" style={{gridTemplateColumns:"1.2fr 1fr 1fr 1fr"}}>
          {["Week","Safety Score","Saved (AED)","AI Match"].map(h=><span key={h} className="tbl-clbl">{h}</span>)}
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"9px"}} className="fade-in">
        {[
          ...
          {label:"AI Match Rate",value:"92.75%",color:"#5599ff"},
        ].map(({label,value,color})=>(
  ...
  return(
    <div className="rs-body">
      ...
      <div className="rs-impact">
        <div style={{fontSize:"10px",fontWeight:"600",letterSpacing:".1em",textTransform:"uppercase",color:"var(--text-2)",marginBottom:"8px"}}>AI Calibration Preview</div>
        <div style={{fontSize:"12px",color:"var(--text-2)",fontStyle:"italic",lineHeight:"1.65",marginBottom:"14px"}}>
          Based on these settings, your AI is now calibrated for <strong style={{color:"var(--text-1)",fontStyle:"normal"}}>your cafe's specialty standards</strong> — using a <strong style={{color:"var(--emerald)",fontStyle:"normal"}}>{buffer}% steaming buffer</strong>, Latte at <strong style={{color:"var(--text-1)",fontStyle:"normal"}}>{drinks.latte.milk}ml</strong>, Flat White at <strong style={{color:"var(--text-1)",fontStyle:"normal"}}>{drinks.fw.milk}ml</strong>, Cappuccino at <strong style={{color:"var(--text-1)",fontStyle:"normal"}}>{drinks.capp.milk}ml</strong>.
        </div>
...
function SignupScreen({ onSignup, onGoLogin }) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      else { setSession(res); onSignup(res); }
...
export default function ProfitGuardAI(){
  ...
  const [screen,   setScreen]   = useState("app");   // "login"|"signup"|"onboarding"|"app"
  const [authSess, setAuthSess] = useState(null);
      ...
      {tab==="dashboard"&&(
        <>
          <div className="pg-header">
            ...
            <div className="pg-subhead">AI-powered procurement for your café — updated in real time.</div>
          ...
          <div className="pg-body">
            ...
            <div className="stat-row fade-in">
              ...
              <div className="stat-card gold"><div className="stat-n">92%</div><div className="stat-l">AI match rate</div></div>
            </div>
            ...
            <div className="fade-in" style={{animationDelay:".16s"}}>
              <div className="pg-sect" style={{color:"#6680aa"}}>AI Recommendation</div>
              <div className="card-white" style={{padding:"18px"}}>
                <div style={{display:"flex",alignItems:"center",gap:"9px",marginBottom:"12px"}}>
                  <div className="ai-chip"><div className="ai-blink"/>AI Insight</div>
            ...
            {/* AI Assistant */}
            <div className="fade-in" style={{animationDelay:".22s"}}>
              <div className="pg-sect" style={{color:"#7a6aaa"}}>Ask AI Assistant</div>
              <AIAssistant/>
            </div>
